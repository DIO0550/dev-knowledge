...
- **A 派: 非装飾コンポーネント**（Radix UI / Tailwind Labs Headless UI / Ariakit など）— `` 等のコンポーネントを提供、styling は利用者
...
- 英語圏主要ライブラリの設計を、実コードベースで確認した上で参照する

## "headless" の共通定義

各ライブラリのドキュメントから:

- TanStack Table: 「Headless UI for building powerful tables & datagrids」「Table does not assume where the data lives」「stays below the visual layer」
  ...
  return (
    <AlertDialog.Root>
      ...
      <AlertDialog.Portal>
        ...
        <AlertDialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded">
          ...
          <div className="mt-4 flex gap-2 justify-end">
            <AlertDialog.Cancel asChild>
              <button className="px-4 py-2 border rounded">キャンセル</button>
            </AlertDialog.Cancel>
...
unstyled なコンポーネント。`data-checked` 等の data 属性で状態を CSS から拾える設計。v2.0 以降は内部で React Aria の hooks を使用している（Tailwind Labs ブログで明言）。
  ...
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <svg
        ...
        aria-hidden="true"
        style={{
          border: isFocusVisible ? '2px solid blue' : '1px solid #ccc',
          borderRadius: 4,
        }}
      >
        <rect
...
`useCheckbox` は `{ inputProps, ... }` を返すだけ。`` を書くのも `` で囲むのも `` で隠して SVG で自前のチェックボックスを描くのも、全て利用者の責任。
...
`useReactTable` は `table` インスタンスを返す。`<table>` を書くのも `flexRender` で渡すのも利用者。`flexRender` は内部的に「columnDef.header が関数なら `` で呼び、そうでなければ値を返す」という分岐をしている（ソース確認済み）。
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div>
        ...
        <input
          ...
          {...register('password', {
            required: 'パスワードは必須です',
            minLength: { value: 8, message: '8 文字以上' },
          })}
        />
...
具体例で示す。`useFormField` 型の純粋ロジック hook なら、同じロジックを `` でも `` でも `` でも使い回せる:
...
逆に `useCheckbox` が `` を返すように作られていたら、MUI Checkbox では使えない、自社 DS では使えない。
  ...
  const field = (
    <div>
      <input
        ...
        onBlur={() => setTouched(true)}
      />
      {touched && !isValid && <p style={{ color: 'red' }}>invalid email</p>}
    </div>
  );
...
export function SomeComplexForm() {
  ...
  return (
    <form>
      <label>Email</label>
      {email.field}
      <button type="submit" disabled={!email.isValid}>送信</button>
    </form>
  );
}
```
