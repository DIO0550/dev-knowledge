...
- **学習・記述コストが低い**：`const [x, setX] = useState(初期値)` だけで始められる。前もって書くコードが少ない。
- **単純な更新では読みやすい**：「この値をこう変える」が `setX(...)` に直接表れ、処理が一直線で追いやすい。
- **局所的**：1 つの値ごとに独立して持てるので、無関係な state を無理に 1 箇所へまとめる必要がない。

**デメリット**

- **更新ロジックが散らばる**：同じ state を複数のイベントハンドラで更新すると、ロジックがコンポーネント中に点在し、全体像を掴みにくい。
- **不正な state を作りやすい**：呼び出し側がどんな値でも `setState` できるため、「ありえない状態の組み合わせ」を防げず、バグの温床になりやすい。
- **関連 state の同期が手作業**：相互に依存する複数の値（例：`status` と `data` と `error`）を別々の `useState` で持つと、更新漏れ・不整合が起きやすい。
- **デバッグの手がかりが少ない**：どこで・なぜその値になったのかを追う共通の入口がない。

### useReducer

**メリット**

- **更新ロジックを 1 箇所に集約**：state の変え方が reducer に集まり、「どの操作でどう変わるか」を一覧できる。
- **変更を action に縛れる**：コンポーネント側は dispatch しかできず、実際の更新は reducer が決める。不正な状態遷移を弾け、状態をステートマシン的に管理できる。
- **how と what happened の分離**：「何が起きたか（action）」と「どう変えるか（reducer）」が分かれ、複雑な更新でも見通しが良い。
- **テストしやすい**：reducer はコンポーネントに依存しない純粋関数なので、単独で export して「この state＋この action → この結果」を直接アサートできる。
- **デバッグしやすい**：reducer に `console.log` を仕込めば、すべての更新とその原因（どの action か）を一括で追える。

**デメリット**

- **ボイラープレートが増える**：reducer 関数と dispatch の両方を書く必要がある。
- **単純なケースでは過剰**：`useState` の方が読みやすい場面で reducer を入れると、かえって複雑になる。
- **デバッグのステップ数が増える**：dispatch → reducer と処理を経由するぶん、`useState` より多くのコードを追う必要がある。
