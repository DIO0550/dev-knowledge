...
- "headless" は **「振る舞い・状態管理・アクセシビリティはライブラリ、見た目（markup と styling）は利用者」** という設計方針。実装機構には大きく 2 つの形がある。
- **A 派: 非装飾コンポーネント**（Radix UI / Tailwind Labs Headless UI / Ariakit など）— `<AlertDialog>` 等のコンポーネントを提供、styling は利用者。
...
- A 派 / B 派は排他ではない。React Aria は hook 版に加えてコンポーネント版（React Aria Components）も提供し、Headless UI v2 は A 派でありながら内部で B 派（React Aria）の hook を使う。
...
- React Aria: 「a library of React Hooks (rather than components) that handle behavior, ARIA semantics, internationalization, and adaptive interactions」。
...
Composable な primitives を組み合わせる。styling は CSS（クラス名やユーティリティ）で。
...
export function SubscribeForm() {
  ...
  return (
    <Field className="flex items-center gap-2">
      ...
      >
        <svg className="hidden group-data-[checked]:block" viewBox="0 0 14 14">
          <path d="M3 8L6 11L11 3" />
        </svg>
      </Checkbox>
...
unstyled なコンポーネント。`data-checked` 等の data 属性で状態を CSS から拾える設計。v2.0 以降は内部で React Aria の hooks を使用している（Tailwind 公式ブログで明言）。
    ...
    <label style={{ display: 'flex', alignItems: 'center' }}>
      ...
      <svg width={24} height={24} aria-hidden="true">
        <rect
          ...
          stroke={isFocusVisible ? 'blue' : 'gray'}
        />
      </svg>
      {props.children}
    </label>
...
`useReactTable` は `table` インスタンスを返す。`<table>` を書くのも `flexRender` で渡すのも利用者。`flexRender` は内部的に「`columnDef.header`/`cell` が **React コンポーネント（クラス / 関数 / exotic component）と判定されれば** `createElement`（= JSX）で呼び、そうでなければ値をそのまま返す」という分岐をしている（判定は単純な `typeof === 'function'` 単体ではなく内部の `isReactComponent` ヘルパーによる）。
...
具体例で示す。`useFormField` 型の純粋ロジック hook なら、同じロジックを `<input>` でも MUI でも自社 DS でも使い回せる:
...
「内部実装としてのカスタムフック」の使い方なら、render hooks 系も問題にならない:
...
devstation の記事「Why You Shouldn't Put JSX in Custom React Hooks」が代表的な批判（個人ブログである点には留意）。要点:

1. **再利用性**: JSX が特定の UI 実装に結びつくため、異なる文脈での再利用が難しい。
2. **パフォーマンス**: JSX が毎レンダーで実行される。
3. **テスト容易性**: hook を単体でテストするのが難しくなる。

結論として「JSX を custom hooks に含めず、必要なデータや state を返すようにし、呼び出し側コンポーネントにレンダリングを任せるべき」と書かれている。これは render hooks への直接的な対抗論で、英語圏ではこちらの立場のほうが優勢。
...
- A 派 / B 派は排他ではなく、React Aria（hook 版 + コンポーネント版）や Headless UI v2（A 派の見た目で内部は B 派 hook）のように両者を併用する設計もある。
...
- Headless UI v2.0 リリース（React Aria 採用を明言）: https://tailwindcss.com/blog/headless-ui-v2
