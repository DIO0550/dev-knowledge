...
良い例 / 悪い例として、次が挙げられている:

```tsx
function BlogPost() {
  return <Layout><Article /></Layout>; // ✅ Good: JSX でのみ使う
}

function BlogPost() {
  return <Layout>{Article()}</Layout>; // 🔴 Bad: 直接呼んではいけない
}
```

そして**「なぜか」の理由**として示されているのは:
...
公式が挙げる、React に呼ばせる（`<A />` 形式の）メリット（"Letting React orchestrate rendering also allows a number of benefits"）:

- **Components become more than functions.** hooks による local state がコンポーネントの identity（tree 上の位置）に結びつく
- **Component types participate in reconciliation.** tree の概念的な構造を React に伝えることになる
- **React can enhance your user experience.** concurrent rendering 等で、コンポーネント呼び出しの合間にブラウザに仕事をさせられる
- **A better debugging story.** / **More efficient reconciliation.** DevTools での introspection や、再レンダー対象の絞り込みが効く
...
uhyo 氏の Qiita 記事でも、コンポーネントをカスタムフックにしたときのフックの帰属について以下のように説明している:
